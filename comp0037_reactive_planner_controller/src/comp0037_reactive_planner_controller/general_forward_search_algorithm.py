...
class GeneralForwardSearchAlgorithm(PlannerBase):
    ...
    def search(self, startCoords, goalCoords):
        ...
        if (self.occupancyGrid.getCell(startCoords[0], startCoords[1]) > 0.5):
            print("start cell status: " + str(self.occupancyGrid.getCell(startCoords[0], startCoords[1])))
            return False

        if (self.occupancyGrid.getCell(goalCoords[0], goalCoords[1]) > 0.5):
            print("end cell status: " + str(self.occupancyGrid.getCell(goalCoords[0], goalCoords[1])))
            return False
